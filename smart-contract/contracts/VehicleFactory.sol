...
contract VehicleFactory is Ownable {
    ...
    modifier hasDeployed() {
        ...
        require(
            isDeployed,
            "The vehicle contract has not been deployed by this factory."
        );
        _;
    }
    ...
    function getOwnedDeployedVehicles(
        address ownerAddress
    ) public view returns (Vehicle[] memory) {
        Vehicle[] memory ownedDeployedVehicles = new Vehicle[](
            deployedVehicles.length
        );
    ...
    function getDeployedVehicleDatas()
        external
        view
        returns (VehicleData[] memory)
    {
        VehicleData[] memory vehicleDatas = new VehicleData[](
            deployedVehicles.length
        );
    ...
    function getOwnedDeployedVehicleDatas(
        address ownerAddress
    ) public view returns (VehicleData[] memory) {
        Vehicle[] memory ownedDeployedVehicles = getOwnedDeployedVehicles(
            ownerAddress
        );

        VehicleData[] memory ownedDeployedVehicleDatas = new VehicleData[](
            ownedDeployedVehicles.length
        );
    ...
    function refundToAuctioneer(
        address previousAuctioneer,
        address vehicleContractAddress,
        uint32 index,
        uint quantity
    ) public hasDeployed {
